...
def check_activity(inactivity_time):
    ...
    while True:
        ...
        if (mouse_moved and not autoclick_active) or keyboard_event or clicked:
            if current_mouse_position != previous_mouse_position: 
                previous_mouse_position = current_mouse_position
            if not autoclick_active: update_label_text(mouse_mover_root,label1,"User is not AFK")
            previous_activity_time = time.time()
        ...
        if elapsed_time >= inactivity_time and not autoclick_active:
            perform_random_actions()
            # previous_mouse_position=current_mouse_position
            # current_mouse_position = mouse.get_position()
            previous_activity_time=current_time
            autoclick_active = True
        elif elapsed_time < inactivity_time and autoclick_active:
